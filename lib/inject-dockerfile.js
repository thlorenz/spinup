 ...
 * @param {boolean} removeRootDir if set the root dir is removed
 * @return {ReadableStream} with the Dockerfile injected 
 */
function injectDockerFile(tarballStream, removeRootDir) {
  var pack = tar.pack()
    , extract = tar.extract();
  
  extract
    .on('entry', function (hdr, stream, cb) {
      if (removeRootDir) hdr.name = removeRootDirFrom(hdr.name);
      inspect(hdr);
...
if (!module.parent && typeof window === 'undefined') {
  ...
  var ins = fs.createReadStream(dir + '/in.tar', 'utf8');
